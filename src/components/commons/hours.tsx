...
const DayRow = (props: DayRow) => {
  ...
  React.useEffect(() => {
    ...
    if (keysFromData) {
      ...
      for (let index = 0; index < keysFromDataUnique.length; index++) {
        ...
        let day = new Date(element).getDate();
       
    //     let a:any,s,holidayDate:any;
    //    function join(t:any, a:any, s:any) {
    //      function format(m:any) {
    //      let f = new Intl.DateTimeFormat('en', m);
    //      return f.format(t);  
    ...
    //   a = [{year: 'numeric'},{month: 'numeric'}, {day: '2-digit'}];   
      // c_specific_day&&c_specific_day.map((boxingday:any)=>{
          
      //     let newdate=join(new Date(element), a, '-')
            
              
      //         holidayarray.forEach((res:any)=>{
           
      //           if(res.date==newdate){
      //           res.holidayname=boxingday.holidayName;
      //         }
      //         })
              
      //         console.log(holidayarray,"kfgnisdbhgf",element)
      //   })
  ...
  console.log(myDataAccordintToMe)
